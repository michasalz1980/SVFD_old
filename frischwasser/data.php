<?php
    ...
    // Optimierte SQL-Abfrage für tägliche Daten
    $sql = 'SELECT
    ...
    // SQL-Abfrage für minütliche Daten (Standard: heute)
    $filterDate = isset($_GET['filter_date']) ? $_GET['filter_date'] : date('Y-m-d');
    $filterDateEnd = isset($_GET['filter_date_end']) ? $_GET['filter_date_end'] : $filterDate;

    $sql_minutely = 'SELECT
            DATE_FORMAT(f.`datetime`, "%d.%m.%Y %H:%i") AS Zeitpunkt,
            f.`datetime` AS datetime_raw,
            f.`counter`/1000 AS `Zählerstand`,
            f.`consumption`/1000 AS `Verbrauch (in m³)`,
            DATE_FORMAT(f.`datetime`, "%H:%i") AS Zeit
        FROM
            `ffd_frischwasser` f
        WHERE
            DATE(f.`datetime`) BETWEEN ? AND ?
        ORDER BY
            f.`datetime` ASC';
    
    $aLastgaengeProMinute = R::getAll($sql_minutely, [$filterDate, $filterDateEnd]);
